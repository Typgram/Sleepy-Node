...
router
  ...
  .get((req, res) => {
    try {
      ...
      if (
        ...
        if (req.query.using === "true") {
          updatedDevice.using = true;
        } else if (req.query.using === "false") {
          updatedDevice.using = false;
        }
      ...
      if (req.query.app_name !== undefined || req.query.status !== undefined) {
        updatedDevice.app_name =
          req.query.app_name !== undefined
            ? req.query.app_name
            : req.query.app_name;
      }
  ...
  .post((req, res) => {
    try {
      ...
      if (!req.body.id) {
        logger.info(
          "POST /api/device/set 400 Bad Request - Missing device ID parameter"
        );
        const errorResponse = APIUnsuccessful(
          400,
          "Missing required parameter 'id'"
      ...
      if (deviceIndex === -1) {
        logger.info(
          `POST /api/device/set 404 Not Found - Device with ID ${deviceId} not found`
        );
        const errorResponse = APIUnsuccessful(
          404,
          `Device with ID ${deviceId} not found`
      ...
      // 更新传入的参数
      if (req.body.show_name !== undefined) {
        updatedDevice.show_name = req.body.show_name;
      }

      if (req.body.using !== undefined && typeof req.body.using === "boolean") {
        updatedDevice.using = req.body.using;
      } else if (req.body.using !== undefined) {
        logger.info(
          `POST /api/device/set 400 Bad Request - Invalid using parameter for device ${deviceId}`
        );
        const errorResponse = APIUnsuccessful(
          400,
          "Argument 'using' must be boolean"
      ...
      if (req.body.app_name !== undefined || req.body.status !== undefined) {
        updatedDevice.app_name =
      ...
      try {
        ...
        logger.info(
          `POST /api/device/set 200 OK - Device ${deviceId} updated successfully`
        );
      ...
      logger.error(
        `POST /api/device/set 500 Internal Server Error: ${error.message}`
      );
      const errorResponse = APIUnsuccessful(
        500,
        "An error occurred while processing your request"
